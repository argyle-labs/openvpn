//! openvpn service backend — OpenVPN server.
//!
//! Implements `ServiceBackend` so the generic `service.*` tools
//! (deploy/backup/restore/configure/status/connect/sync) drive openvpn. No
...
/// openvpn backend. Holds only the provider name; per-instance endpoint/creds
...
impl ServiceBackend for OpenvpnBackend {
    fn provider(&self) -> &str { self.provider }

    /// Runtimes openvpn can be placed on. `service.deploy` hands the
    ...
    fn default_port(&self) -> u16 { 1194 }

    /// In-workload paths holding config/data. This is ALL openvpn declares for
    ...
    {
        // TODO: describe the openvpn workload (image/template, ports, mounts,
        ...
        Box::pin(async move { Err(ServiceError::unimplemented("openvpn.workload_spec")) })
    }

    fn configure<'a>(&'a self, _ep: &'a Endpoint, _config: &'a str)
        -> BoxFuture<'a, Result<(), ServiceError>>
    {
        // TODO: apply openvpn-specific config idempotently.
        Box::pin(async move { Err(ServiceError::unimplemented("openvpn.configure")) })
    }
        ...
        -> BoxFuture<'a, Result<ServiceStatus, ServiceError>>
    {
        ...
        Box::pin(async move { Err(ServiceError::unimplemented("openvpn.status")) })
    }
}
...
mod tests {
    ...
    fn declares_provider() {
        let b = OpenvpnBackend::new("openvpn");
        assert_eq!(b.provider(), "openvpn");
    }
}
